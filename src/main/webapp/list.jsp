        ...
        <nav aria-label="navigation for courses">
            <ul class="pagination">
                <c:if test="${currentPage != 1}">
                    <li class="page-item">
                        <a class="page-link" href="list?page=${currentPage - 1}<c:if test="${theme != null}">&theme=${theme}</c:if>">Previous</a>
                    </li>
                </c:if>
                ...
                <c:forEach begin="1" end="${noOfPages}" var="i">
                    <c:choose>
                        <c:when test="${currentPage eq i}">
                            <li class="page-item">
                                    <%--                        ${i}--%>
                                <a class="page-link" href="list?page=${i}<c:if test="${theme != null}">&theme=${theme}</c:if>">${i}</a>
                            </li>
                        </c:when>
                        <c:otherwise>
                            <li class="page-item">
                ...
                <c:if test="${currentPage lt noOfPages}">
                    <li class="page-item">
                        <a class="page-link" href="list?page=${currentPage + 1}<c:if test="${theme != null}">&theme=${theme}</c:if>">
                            Next
                        </a>
                    </li>
                </c:if>
